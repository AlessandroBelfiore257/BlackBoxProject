...
void obdConfig() {
  ...
  if (!myELM327.begin(SerialBT, true, 5000)) {
...
void profilazioneCallback() {
  ...
  // Accelerazione / decelerazioni brusche
  if (x > ACCELERAZIONE_BRUSCA || x < (DECELERAZIONE_BRUSCA)) {
    nRilevamentiAccDecBrusche++;
  }
  // Sterzate sinistra e destra brusche
  if (y > STERZATA_SX_BRUSCA || y < (STERZATA_DX_BRUSCA)) {
