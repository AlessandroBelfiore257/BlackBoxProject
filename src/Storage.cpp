...
int getValueFromT5(const char *problem, String column) {
  if (db_open(PATH_STORAGE_DATA_DB, &db) != SQLITE_OK) {
    return -1;
  }
  char sql[256];
  sqlite3_stmt *stmt;
  snprintf(sql, sizeof(sql), "SELECT %s FROM t5 WHERE problema = ?;", column.c_str());
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) {
    #ifdef DEBUG
    Serial.printf("Failed to prepare statement: %s\n", sqlite3_errmsg(db));
    #endif
...
void updateColumnValueIntoT5(const char *problem, String column, int value) {
  ...
  char sql[256];
  sqlite3_stmt *stmt;
  snprintf(sql, sizeof(sql), "UPDATE t5 SET %s = ? WHERE problema = ?;", column.c_str());
